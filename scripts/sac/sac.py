...
class SoftActorCritic():
    ...
    def __init__(self, env_name):
        ...
        """         
        self.env = gym.make(env_name, train=True, noise_required=True) 
        self.test_env = gym.make(env_name, train=False, noise_required=True) 
    ...
    def load_weights(self, critic1_path, critic2_path, policy_path):
        self.critic1.load_save_dict(torch.load(critic1_path))
        self.target_critic1.load_save_dict(torch.load(critic1_path))

        self.critic2.load_save_dict(torch.load(critic2_path))
        self.target_critic2.load_save_dict(torch.load(critic2_path))

        self.policy.load_save_dict(torch.load(policy_path))
    ...
    def test(self, policy_path, args):
        ...
        for test_ep_idx in range(args.num_test_episodes):
            ...
            distance_remaining.append(info["fv_pos"] - info["ego_pos"])
            test_episodes_returns.append(episode_return)

        return test_episodes_returns, distance_remaining 
    ...
    def visualize(self, policy_path, args):
        ...
        while not done: 
            ...
            rel_dist_list.append(info["fv_pos"] - info["ego_pos"])
            rel_vel_list.append(info["fv_vel"] - info["ego_vel"])
        ...
        axes[0, 0].plot(rel_dist_list)
        axes[0, 1].plot(rel_vel_list)
        axes[0, 2].plot(total_reward_list)
        ...
        axes[0, 0].title.set_text('Distance between vehicles over time')
        axes[0, 1].title.set_text('Velocity between vehicles over time')
        axes[0, 2].title.set_text('Total reward accumulated over time')
        ...
        axes[0, 0].set_ylabel('Rel dist (m)')
        axes[0, 1].set_ylabel('Rel vel (m/s)')
