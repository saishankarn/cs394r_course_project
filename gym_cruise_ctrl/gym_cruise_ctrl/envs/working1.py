...
class CruiseCtrlEnv(gym.Env):
	...
	def __init__(self):
		...
		self.max_vel_possible = self.fv_min_vel + self.max_acc * self.max_episode_steps
	...
	def step(self, action):
		...
		self.fv_dist += self.fv_min_vel*self.delt 
		self.fv_dist -= dist_traveled
		#print("code value : ", self.fv_dist)

		next_state = np.array([(self.ego_vel - self.fv_min_vel)/self.max_vel_possible, self.fv_dist/self.fv_start_dist], dtype=np.float32)

		# reward for the state transition
		reward = (dist_traveled - self.fv_min_vel*self.delt) / 100
