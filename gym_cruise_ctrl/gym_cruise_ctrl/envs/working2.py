...
class CruiseCtrlEnv(gym.Env):
	...
	def __init__(self):
		...
		self.fv_start_dist = 200 
		self.fv_dist = 0

		# ego vehicle variables
		self.ego_vel = self.fv_min_vel
	...
	def step(self, action):
		...
		acc = action 
		dist_traveled_ego = self.ego_vel*self.delt + 0.5*acc
		dist_traveled_fv = self.fv_vel*self.delt
		dist_traveled = dist_traveled_ego - dist_traveled_fv
		...
		next_state = np.array([(self.ego_vel -self.fv_vel)/self.max_vel_possible, self.fv_dist/self.fv_start_dist], dtype=np.float32)

		# reward for the state transition
		reward = dist_traveled / 1000
	...
	def reset(self):
		...
		self.fv_dist = max(20*np.random.randn() + 100, 10) # (100 +- 20)m
		self.fv_vel = min(self.fv_max_vel, max(5*np.random.randn() + 25, self.fv_min_vel))	
		self.ego_vel = max(5*np.random.randn() + 10, 0)
		self.done = False
		self.episode_steps = 0
		state = np.array([(self.ego_vel-self.fv_vel)/self.max_vel_possible, self.fv_dist/self.fv_start_dist], dtype=np.float32)
